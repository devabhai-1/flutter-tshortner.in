...
function Wallet() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Reset form - keep method/account, reset amount
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className={styles.field}>
                ...
                <span className={styles.hintText}>
                  Sirf 2 methods available: <b>UPI</b> India ke liye, aur <b>Binance USDT (TRC20)</b> global ke liye.
                </span>
              </div>

              <div className={styles.field}>
                <label htmlFor="account">UPI ID / Binance UID / Wallet Address</label>
                <input
                  id="account"
                  type="text"
                  value={account}
                  onChange={(e) => setAccount(e.target.value)}
                  placeholder="example@upi / Binance UID / TRC20 address"
                  required
                />
                <span className={styles.hintText}>Jo method select kiya hai uska sahi ID yahan likho.</span>
              </div>
                  ...
                  {history.length === 0 ? (
                    ...
                    history.map((item) => (
                      <tr key={item.id || item.createdAt}>
                        ...
                        <td>{item.method || "-"}</td>
                        <td><span style={{ fontSize: '0.72rem', color: '#9ca3af' }}>{item.account || "-"}</span></td>
                        <td>
